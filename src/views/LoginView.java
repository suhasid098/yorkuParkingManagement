...
public class LoginView extends JFrame {
	...
	public LoginView(Main frame) {
		...
		this.fieldPane.add(Box.createVerticalGlue()); // Makes components move with the top of the pane. Used in
		...
		this.fieldPane.add(Box.createVerticalGlue()); // Makes components move with the bottom of the pane. Used in
														// centering components with resizing.

		// Add buttons to button panel.
		buttonPane.add(Box.createHorizontalGlue()); // Makes components keep with the right of the pane. Used in
													// centering components with resizing.
		...
		buttonPane.add(Box.createHorizontalGlue()); // Makes components keep with the left of the pane. Used in
													// centering components with resizing.
		...
		this.saveButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) { // loggedInuser is set in UserController
				...
				if (thisView.emailField.getText().contains("@")) {
					result = UserController.logInUser(thisView.emailField.getText(),
							new String(thisView.passField.getPassword()));
					user = true;
				} else {
					result = ManagerController.logInManager(thisView.emailField.getText(),
							new String(thisView.passField.getPassword()));
					user = false;
				}
