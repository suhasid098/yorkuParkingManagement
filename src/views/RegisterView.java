...
public class RegisterView extends LoginView {
	...
	public RegisterView(Main previousFrame) {
		...
		this.fieldPane.add(Box.createRigidArea(new Dimension(0, sameSectionSpace)), pos); // Space between label and
																							// field.
		this.fieldPane.add(nameLabel, pos);

		// Add components before error label.
		pos = 3;
		this.fieldPane.add(Box.createRigidArea(new Dimension(0, diffSectionSpace)),
				fieldPane.getComponentCount() - pos); // Space between section.
		this.fieldPane.add(confirmLabel, fieldPane.getComponentCount() - pos); // Adds name label to pane.
		this.fieldPane.add(Box.createRigidArea(new Dimension(0, sameSectionSpace)),
				fieldPane.getComponentCount() - pos); // Space between label and field.
		...
		this.saveButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String result = UserController.registerUser(thisView.nameField.getText(), thisView.emailField.getText(),
						new String(thisView.passField.getPassword()), new String(thisView.confirmField.getPassword()),
						buttonText);
