...
public class ManageAccountsView extends JFrame {
	...
	private void listSetup() {
		for (User user : UserController.getUnapprovedUsers()) {
			listModel.addElement(
					"Type: " + user.getAccountType() + "    Email: " + user.getEmail() + "    Name: " + user.getName());
		}

		// Set up List dimensions.
		accountList.setPrototypeCellValue(
				"Using this to set the list cell width because I don't know of a better way to do it.");
	...
	public ManageAccountsView(Main frame) {
		...
		listPane.add(Box.createVerticalGlue()); // Makes components move with the top of the pane. Used in centering
		...
		listPane.add(Box.createRigidArea(new Dimension(0, componentSeperation)));
		listPane.add(Box.createVerticalGlue()); // Makes components move with the bottom of the pane. Used in centering
												// components with resizing.
		...
		accountList.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent e) {
				if (!e.getValueIsAdjusting()) {
					...
					// Determine selected list item and disable or enable the lot that corresponds
					// to that item.
		...
		accountButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (manager != null) {
					accountLabel.setText("<html><center>Username: " + manager.getName() + "<br>Password: "
							+ manager.getPassword() + "</center></html>");
				}
			}
		});
	}
